...
describe("Game item markup", () => {
  ...
  it("check title content is the same as in props", () => {
    expect(defaultGame.find('[data-test-id="title"]').text()).toEqual(
      props.title
    );
  });
  ...
  it("rating is the same as in props", () => {
    ...
    expect(game.find('[data-test-id="rating"]').text()).toEqual(
      rating.toString()
    );
  });
  ...
  it("Match Snapshot", () => {
    const game = shallow(
      <Game {...props} rating={4} description="Desc" genres={["1", "2", "3"]} />
    );
